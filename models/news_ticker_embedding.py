...
query = f"""SELECT nws.id, nws.text, nws.ticker, dsc.name, dsc.sector FROM
            news_with_ticker nws LEFT JOIN ticker_describe dsc ON nws.ticker = dsc.ticker
            WHERE nws.id NOT IN (SELECT id FROM news_with_ticker_embedd)
            """
with psycopg2.connect(dbname = DB_NAME, user = DB_USER, password = DB_PASS, host = DB_HOST, port = DB_PORT) as conn:
    with conn.cursor() as cur:
        cur.execute(query)
        news = pd.DataFrame(cur.fetchall(), columns=["id", "text", "ticker", "name", "sector"])

for index in tqdm(range(len(news))):
    news.loc[index, "text"] = clean_text(news.loc[index, "text"])
...
for index in tqdm(range(len(news))):
    ...
    s = news["sector"][index]
    m = news["text"][index]
    news.loc[index, "text"] = f"[{t}, {n}, {s}][SEP]{m}"
...
query = "INSERT INTO news_with_ticker_embedd VALUES %s"
with psycopg2.connect(dbname = DB_NAME, user = DB_USER, password = DB_PASS, host = DB_HOST, port = DB_PORT) as conn:
    with conn.cursor() as cur:
